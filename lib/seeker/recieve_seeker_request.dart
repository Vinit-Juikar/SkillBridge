...
class RecieveSeekerRequests extends StatefulWidget {
  const RecieveSeekerRequests({super.key});
...
class _RecieveSeekerRequestsState extends State<RecieveSeekerRequests> {
  ...
  Widget build(BuildContext context) {
    ...
    CollectionReference users =
        FirebaseFirestore.instance.collection('seekerInformation');
    DocumentReference docRef = users.doc(number);
    docRef.get().then((DocumentSnapshot documentSnapshot) {
      if (documentSnapshot.exists) {
        myList = documentSnapshot.get('RList');
        print(myList);
      } else {}
    });
...
class RecieveRequestFormat extends StatefulWidget {
  var SeekerRequestId;
  RecieveRequestFormat(this.SeekerRequestId, {super.key});
...
class _RecieveRequestFormatState extends State<RecieveRequestFormat> {
  ...
  Widget build(BuildContext context) {
    ...
    number = user?.phoneNumber ?? 'User email';
    CollectionReference users =
        FirebaseFirestore.instance.collection('seekerRequests');
    DocumentReference docRef = users.doc(widget.SeekerRequestId);
    docRef.get().then((DocumentSnapshot documentSnapshot) {
      if (documentSnapshot.exists) {
